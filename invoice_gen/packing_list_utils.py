...
def calculate_rows_to_generate(packing_list_data: dict, sheet_config: dict) -> int:
    ...
    Args:
        ...
        sheet_config: The configuration for the sheet (not directly used here but
    ...
    # Assumption: Each table has a 2-row header and a 1-row footer.
    header_rows_per_table = 2
    footer_rows_per_table = 1
    # A blank row is added between tables for spacing.
    spacing_rows = max(0, num_tables - 1)
    # A grand total row is added if there is more than one table.
    grand_total_rows = 1 if num_tables > 1 else 0

    # Sum of all data rows across all tables.
...
def generate_full_packing_list(worksheet: Worksheet, start_row: int, packing_list_data: dict, sheet_config: dict):
    ...
    # --- Lists to track row locations for final styling ---
    all_data_ranges = []
    all_header_infos = []
    all_footer_rows = []
    ...
    # --- MAIN LOOP: Build one table at a time ---
    for i, table_key in enumerate(table_keys):
        ...
        all_header_infos.append(header_info) # Track header location
        write_pointer_row = header_info['second_row_index'] + 1
        ...
        # 2. WRITE DATA and CALL STYLE UTILITY FOR EACH CELL
        ...
        all_footer_rows.append(write_pointer_row) # Track footer location
        write_pointer_row += 1

        if i < num_tables - 1: write_pointer_row += 1

    # --- AFTER LOOP ---
    if num_tables > 1:
        invoice_utils.write_footer_row(worksheet, write_pointer_row, all_header_infos[-1], all_data_ranges, footer_config, grand_total_pallets, "GRAND TOTAL:")
        all_footer_rows.append(write_pointer_row) # Track grand total footer location
